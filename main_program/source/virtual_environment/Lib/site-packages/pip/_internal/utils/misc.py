...
def __url(
    url: str, _netloc: Callable[[str], Tuple[Any, ...]]
) -> Tuple[str, NetlocTuple]:
    """ and replace netloc in a url.

    _netloc is a function taking the netloc and returning a
    tuple. The first element of this tuple is the new netloc. The
    entire tuple is returned.

    Returns a tuple containing the ed url as item 0 and the
    original tuple returned by _netloc as item 1.
    """
    purl = urllib.parse.urlsplit(url)
    netloc_tuple = _netloc(purl.netloc)
    # stripped url
...
def split_auth_netloc_from_url(url: str) -> Tuple[str, str, Tuple[str, str]]:
    ...
    """
    url_without_auth, (netloc, auth) = __url(url, _get_netloc)
...
def remove_auth_from_url(url: str) -> str:
    ...
    return __url(url, _get_netloc)[0]


def redact_auth_from_url(url: str) -> str:
    """Replace the password in a given url with ****."""
    return __url(url, _redact_netloc)[0]
